...
const getDashboardData = async (currentAdmin) => {
  try {
    // Dashboard is now admin-only, so we don't need to check role here
    // as it's already checked in the AdminJS configuration
    const isAdmin = currentAdmin.role === 'admin';

    if (!isAdmin) {
      throw new Error('Access denied. Dashboard is only available for administrators.');
    }
